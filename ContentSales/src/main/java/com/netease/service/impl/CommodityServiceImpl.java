...
public class CommodityServiceImpl implements CommodityService {
	...
	public String insert(Commodity commodity) {
		...
		/*
		 * Date nowdate=new Date(); SimpleDateFormat df = new SimpleDateFormat(
	...
	/*
	 * 对一个商品进行修改、更新操作，必须通过事务管理来控制， 此处的逻辑是：对一个商品更新，不过是增加一个一条记录，之前的记录不直接删除，保留更改。
	 * 不过造成的问题就是： 所有跟此记录相关的所有物品都要被更新，比如：购买此物品的用户的交易记录，这个时候需要更改该记录 相比较如
	 * 直接修改记录，之前的记录就看不到，谁操作的就更加难以看到，此处主要看应用场景。
	 * 
	 * 参考淘宝，淘宝里很多宝贝 过几年后就下架了，单依旧在用户的购买记录中，并且用户点击该商品依旧可以看到一些。
	 * 
	 * @see com.netease.service.CommodityService#updateByPrimaryId(com.bean.
	 * Commodity)
	 */
